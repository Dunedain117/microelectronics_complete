...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
  ...
  G4double boxSize = 3.*um;
  G4Box* box = new G4Box("Box", boxSize/2, boxSize/2, boxSize/2);
  G4LogicalVolume* boxLV = new G4LogicalVolume(box, fMaterial, "Box");


  int counter = 0;
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      G4double z = (i-1)* boxSize;
      G4double y = (j-1)*   boxSize;
      G4ThreeVector pos(0, y, z);
      std::ostringstream oss;
      oss << "Box" << counter;
      counter++;
      new G4PVPlacement(0, pos, boxLV, oss.str(), flogicTarget, false, i*3 + j);
    }
  }
  ...
  fLogicWorld->SetUserLimits(userLimits);
  flogicTarget->SetUserLimits(userLimits);
